...
**English**
> - Signing flow: pick a PDF → enter the signature PIN → hold the My Number Card to the top of the iPhone → confirm a signed PDF is produced.
> - Verify flow: open a signed PDF and confirm the signature value, tamper check, and certificate-chain results are shown correctly.
> - Registry-conformance ("登記適合") check display.
> - The "Sign" button's appearance/behavior and the Japanese/English localization.
> - Note: signing requires a physical My Number Card and an NFC-capable iPhone (iOS 17+).
